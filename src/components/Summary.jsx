...
export default function Summary() {
  ...
  return (
    <div className="min-h-screen">
      <header className="px-5 pb-16 pt-20 text-center sm:px-8 xl:px-16 xl:pt-28"><p className="editorial-label">05 Reflection</p><h1 className="mt-5 font-display text-[clamp(3rem,7vw,7rem)] font-medium leading-[0.98] tracking-[-0.02em]">Tổng kết &amp; Suy ngẫm</h1><p className="mx-auto mt-7 max-w-2xl text-lg leading-8 text-muted-dark">Nhìn lại hành trình học tập, những khó khăn đã trải qua và cam kết hoàn thiện Portfolio một cách trung thực.</p></header>
      <main className="mx-auto max-w-5xl px-5 pb-28 sm:px-8">
        <div className="space-y-10">{sections.map((section, index) => <section key={section.number} className={`glass-card grid gap-6 rounded-[2.25rem] p-7 md:grid-cols-[96px_1fr] md:p-10 ${index === 1 ? 'md:translate-x-8' : index === 2 ? 'md:-translate-x-8' : ''}`}><span className="font-display text-6xl font-medium text-primary/35">{section.number}</span><div><p className="editorial-label">{section.label}</p><h2 className="mt-3 font-display text-3xl font-medium tracking-[-0.02em]">{section.title}</h2><p className="mt-7 whitespace-pre-line text-base leading-8 text-charcoal">{section.copy}</p></div></section>)}</div>
        <section className="glass-dark mt-16 rounded-[2.5rem] p-8 sm:p-12"><p className="dark-editorial-label">Academic Integrity</p><h2 className="mt-4 font-display text-4xl font-medium tracking-[-0.02em] text-white">Cam kết Liêm chính Học thuật</h2><p className="mt-7 whitespace-pre-line text-base italic leading-8 text-white/72">{`Em, Trần Trường Giang, cam kết rằng các báo cáo và nội dung trong portfolio này phản ánh đúng quá trình học tập và thực hành của bản thân. Trong quá trình làm, em có sử dụng AI như một công cụ hỗ trợ, chủ yếu để gợi ý cách trình bày, hỗ trợ sửa code và kiểm tra lại một số phần nội dung.
...
Em cảm ơn thầy cô đã dành thời gian xem portfolio này.`}</p><div className="mt-10 border-t border-white/10 pt-6"><p className="dark-editorial-label">Student</p><p className="mt-2 font-display text-2xl font-medium text-white">{student.name}</p></div></section>
      </main>
    </div>
  );
}
