    ...
    <div className="min-h-screen">
      ...
      <section className="px-5 pb-28 sm:px-8 xl:px-16">
        <div className="glass-panel mx-auto max-w-[1280px] overflow-hidden rounded-[2.5rem]">
          <div className="divide-y divide-border-light md:hidden">{projects.map((project) => <article key={project.id} className="p-6 transition-colors hover:bg-white/35"><div className="flex items-start justify-between gap-4"><div><p className="editorial-label">Assignment {String(project.id).padStart(2, '0')}</p><h2 className="mt-3 font-display text-2xl font-medium">{project.title}</h2></div><StatusBadge project={project} /></div><div className="mt-6"><EvidenceLink value={project.report} label="Xem báo cáo" type="pdf" /></div></article>)}</div>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[640px] border-collapse text-left"><thead><tr className="border-b border-border-light bg-white/30 text-[11px] font-extrabold uppercase tracking-[0.14em] text-muted-dark"><th className="px-7 py-6">Assignment</th><th className="px-7 py-6">Report File</th><th className="px-7 py-6">Status</th></tr></thead><tbody>{projects.map((project) => <tr key={project.id} className="border-b border-border-light transition duration-200 hover:bg-white/55"><td className="px-7 py-8"><p className="font-display text-2xl font-medium">{project.title}</p><p className="mt-2 text-xs uppercase tracking-wider text-muted-dark">{project.chapter}</p></td><td className="px-7 py-8"><EvidenceLink value={project.report} label="Xem báo cáo" type="pdf" /></td><td className="px-7 py-8"><StatusBadge project={project} /></td></tr>)}</tbody></table></div>
        ...
        <div className="mx-auto mt-12 grid max-w-[1280px] gap-5 md:grid-cols-3"><div className="glass-card rounded-[2rem] p-8 md:col-span-2"><p className="editorial-label">Integrity Check</p><h2 className="mt-5 font-display text-3xl font-medium">Submission Accuracy</h2><p className="mt-5 max-w-xl text-sm leading-7 text-muted-dark">Mỗi trạng thái được tính trực tiếp từ dữ liệu báo cáo hiện có.</p><p className="mt-10 font-display text-7xl font-medium">{Math.round((completed / projects.length) * 100)}%</p></div><div className="glass-card rounded-[2rem] p-8"><p className="editorial-label">Archive Health</p><p className="mt-10 font-display text-6xl font-medium">{projects.length}</p><p className="mt-2 text-xs uppercase tracking-widest text-muted-dark">assignment records</p></div></div>
      </section>
    </div>
  );
}
