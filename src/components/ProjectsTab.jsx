...
export default function ProjectsTab() {
  ...
  return (
    <div>
      ...
      <section className="mx-auto grid max-w-[1280px] grid-cols-1 gap-6 px-5 pb-28 sm:px-8 lg:grid-cols-2 xl:grid-cols-3 xl:px-16">
        {projects.map((project) => <AssignmentCard key={project.id} project={project} onSelect={setSelectedProject} />)}
      </section>
      <section className="px-5 py-20 sm:px-8 xl:px-16">
        <div className="glass-panel mx-auto grid max-w-[1280px] gap-10 rounded-[2.5rem] p-8 lg:grid-cols-2 lg:items-center lg:p-10"><div><p className="editorial-label">Academic Standards</p><h2 className="mt-4 font-display text-4xl font-medium tracking-[-0.02em]">Evidence before decoration.</h2><p className="mt-6 max-w-xl text-base leading-8 text-muted-dark">Mỗi hồ sơ ưu tiên tính chính xác của nội dung, khả năng truy xuất tài nguyên và sự minh bạch trong quá trình thực hiện.</p></div><div className="rounded-[2rem] bg-white/55 p-10 shadow-glass backdrop-blur-[30px]"><p className="editorial-label">Archive Scope</p><p className="mt-5 font-display text-7xl font-medium">{String(projects.length).padStart(2, '0')}</p><p className="mt-2 text-sm text-muted-dark">documented assignments</p></div></div>
      </section>
    </div>
  );
}
